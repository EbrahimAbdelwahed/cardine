...
TUTOR_DECISION_INSTRUCTION = (
    ...
    "Use assistant_message for greetings, thanks, closures, and conversational guidance "
    "that does not assert unsupported study facts; select an advertised grounded "
    ...
    "acknowledge the request. Ask one concise learner question when the goal is ambiguous; "
    "ask_learner is the only clarification path. Every ordinary learner turn must produce "
    "a presentation-producing decision. Stop is unavailable to this chat policy. If a "
    "requested fact is unsafe or cannot be grounded, ask the learner for a source or "
    "respond without unsupported facts. When a "
    ...
    "and conform its response to the advertised schema. Never use assistant_message, "
